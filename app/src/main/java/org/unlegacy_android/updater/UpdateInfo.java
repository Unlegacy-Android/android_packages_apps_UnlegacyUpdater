...
public class UpdateInfo implements Parcelable, Serializable {
    ...
    public boolean isUpdatable() {
        boolean isDevice = Build.DEVICE.equals(device);
        boolean isNewVersion = (new BigDecimal(Build.VERSION.RELEASE.replace(".","")).compareTo(new BigDecimal(version.replace(".",""))) <= 1 )? true : false;
        boolean isLatestDate = Build.TIME < date;
        if ( (isDevice && isNewVersion && !isLatestDate) ||
                (isDevice && !isNewVersion && isLatestDate))
            return true;
        return false;
    }

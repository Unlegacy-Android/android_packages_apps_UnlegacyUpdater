...
import static org.unlegacy_android.updater.Utils.LAST_STATE;
import static org.unlegacy_android.updater.Utils.LAST_UPDATE_CHECK_PREF;

public class SystemUpdateActivity extends PreferenceActivity {

    private static final String TAG = SystemUpdateActivity.class.getName();

    private PreferenceScreen mScreen;
    private PreferenceCategory mMainCat;
    private Preference mInfoPref;
    private ButtonPreference mCheckUpdatesPref;
    ...
    private Runnable downloadProgress = new Runnable() {
        public void run() {
            ...
            if ( cursor == null || !cursor.moveToFirst() ) {
                // SystemUpdateReceiver has likely already removed the download
            ...
            switch (dStatus) {
                ...
                case DownloadManager.STATUS_RUNNING:
                    ...
                    if ( totalBytes < 0 ) {
                        ...
                        mCheckUpdatesPref.setSummary(strPercent);
                    }
                    break;
    ...
    public static String getDeviceInformation() {
        ...
        deviceInfo
                ...
                .append("Build Date: ").append(Utils.getDate(Build.TIME, "yyyy-MM-dd hh:mm")).append("\n")
                .append("Build Version: ").append(Build.VERSION.RELEASE).append("\n")
                .append("Build Security Patch: ").append(Build.VERSION.SECURITY_PATCH).append("\n")
                .append("Build Number: ").append(Build.VERSION.INCREMENTAL);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMainCat = new PreferenceCategory(this);
        mMainCat.setTitle("Unlegacy Android");
        mScreen.addPreference(mMainCat);

        // Information preference
        mInfoPref = new Preference(this);
        mInfoPref.setTitle("Information");
        mInfoPref.setSummary(getDeviceInformation());
        mMainCat.addPreference(mInfoPref);

        // Check for updates preference
        mCheckUpdatesPref = new ButtonPreference(this);
        mCheckUpdatesPref.setTitle("Check Updates");
        mCheckUpdatesPref.setSummary("");
        mMainCat.addPreference(mCheckUpdatesPref);
    ...
    protected void onStart() {
        ...
        broadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ( SystemUpdateService.ACTION_UPDATE_CHECK_FINISHED.equals(action) ) {
                    checkUpdateFinished(intent);
                } else if ( SystemUpdateService.ACTION_UPDATE_CHECK_FAILED.equals(action) ) {
                    checkUpdateFailed(intent);
                } else if ( SystemUpdateReceiver.ACTION_DOWNLOAD_STARTED.equals(action) ) {
                    updateDownloadStarted(intent);
                } else if ( DownloadManager.ACTION_DOWNLOAD_COMPLETE.equals(action) ) {
                    updateDownloadCompleted(intent);
                } else if ( SystemUpdateReceiver.ACTION_INSTALL_PREPARED.equals(action) ) {
        ...
        intentFilter.addAction(SystemUpdateService.ACTION_UPDATE_CHECK_FINISHED);
        intentFilter.addAction(SystemUpdateService.ACTION_UPDATE_CHECK_FAILED);
        intentFilter.addAction(SystemUpdateReceiver.ACTION_DOWNLOAD_STARTED);
        intentFilter.addAction(DownloadManager.ACTION_DOWNLOAD_COMPLETE);
        intentFilter.addAction(SystemUpdateReceiver.ACTION_INSTALL_PREPARED);
    }
    ...
    */
    ...
    private void checkUpdate() {
        ...
        Intent checkIntent = new Intent(SystemUpdateActivity.this, SystemUpdateService.class);
        checkIntent.setAction(SystemUpdateService.ACTION_UPDATE_CHECK);
        startService(checkIntent);
    }

    private void checkUpdateCancel() {
        setState(State.UNCHECKED);
        Intent cancelIntent = new Intent(SystemUpdateActivity.this, SystemUpdateService.class);
        cancelIntent.setAction(SystemUpdateService.ACTION_UPDATE_CHECK_CANCEL);
        stopService(cancelIntent);
    }

    private void checkUpdateFinished(Intent intent) {
        UpdateInfo updateInfo = intent.getParcelableExtra(EXTRA_UPDATE_INFO);
        if ( updateInfo != null ) {
    ...
    private void updateDownload() {
        ...
        // Start the download
        Intent intent = new Intent(SystemUpdateActivity.this, SystemUpdateReceiver.class);
        intent.setAction(SystemUpdateReceiver.ACTION_START_DOWNLOAD);
    ...
    private void updateInstall() {
        ...
        installIntent.putExtra(SystemUpdateReceiver.EXTRA_FILEPATH, downloadPath);
    ...
    private void setUiViews(
        ...
        mCheckUpdatesPref.setTitle(strTitle);
        mCheckUpdatesPref.setSummary(strStatus);
        mCheckUpdatesPref.setButtonText(strActionButton);
        mCheckUpdatesPref.setButtonVisibility((strActionButton.length() > 0 ? View.VISIBLE : View.GONE));
    ...
    private void setState(final State state) {
        ...
        try {
            this.mCheckUpdatesPref.setButtonOnClickListener(new View.OnClickListener() {
                @Override
